...
def scrape_weather():
    ...
    # 현재 OO℃ (최저 OO℃ / 최고 OO℃)
    curr_temp = soup.find("p", attrs={
        "class": "info_temperature"
    }).get_text().replace("도씨", "")
    min_temp = soup.find("span", attrs={"class": "min"}).get_text()
    max_temp = soup.find("span", attrs={"class": "max"}).get_text()
    # 오전 강수확률 OO% / 오후 강수확률 OO%
    morning_rain_rate = soup.find("span",
                                  attrs={
                                      "class": "point_time morning"
                                  }).get_text().strip()
    afternoon_rain_rate = soup.find("span",
                                    attrs={
                                        "class": "point_time afternoon"
                                    }).get_text().strip()
    # 미세먼지 OO㎍/㎥좋음
    # 초미세먼지 OO㎍/㎥좋음
...
def scrape_headline_news():
    ...
    news_list = soup.find("ul", attrs={
        "class": "hdline_article_list"
    }).find_all("li", limit=3)  # limit = 3 으로 지정하면 li tag가 여러 개 있어도 3개까지만 가져옴.
...
def scrape_it_news():
    ...
    soup = create_soup(url)
    news_list = soup.find("ul", attrs={
        "class": "type06_headline"
    }).find_all("li", limit=5)
    for index, news in enumerate(news_list):
        a_idx = 0
        img = news.find("img")
        if img:
            a_idx = 1  # img tag가 있으면 index 1의 a tag 사용
...
def scrape_today_english():
    ...
    for sentence in sentences[len(sentences) //
                              2:]:  # 8문장이 있다고 가정할 때, index 4~7까지 잘라서 가져옴
        print(sentence.get_text().strip())
    print()

    print("(한글 지문)")
    for sentence in sentences[:len(sentences) //
                              2]:  # 8문장이 있다고 가정할 때, index 0~3까지 잘라서 가져옴
        print(sentence.get_text().strip())
    print()
...
if __name__ == "__main__":
    ...
    scrape_today_english()  # 해커스 오늘의 영어회화 지문 가져오기
